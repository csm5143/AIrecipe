...
// ============ 扫码相关（云开发移除后暂停）============
// 保留接口签名，后续接入第三方 AI 识别后可启用

const SCAN_COUNT_KEY = 'scan_count';
const SCAN_MAX = 3;

function getScanCount(): number {
  return parseInt(wx.getStorageSync(SCAN_COUNT_KEY) || '0', 10);
}

function saveScanCount(count: number): void {
  wx.setStorageSync(SCAN_COUNT_KEY, String(count));
}
...
/** 检查扫码权限（云开发移除后不限制） */
export function checkScanAccess(): ScanAccessResult {
  return { canUse: true, remaining: SCAN_MAX };
}

/** 消耗扫码次数 */
export function consumeScanCountIfNeeded(): number {
  const count = getScanCount();
  const remaining = Math.max(0, SCAN_MAX - count);
  saveScanCount(count + 1);
  return remaining;
}

/** 获取剩余扫码次数 */
export function getDisplayRemainingCount(): number {
  return Math.max(0, SCAN_MAX - getScanCount());
}
